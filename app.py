...
def generate_plots(N, mu, sigma2, S):
    ...
    Y = mu + np.sqrt(sigma2) * np.random.randn(N, 1)  # Replace with code to generate random values for Y with specified mean and variance

    # TODO 2: Fit a linear regression model to X and Y
    # Hint: Use Scikit Learn
    model = LinearRegression()
    model.fit(X,Y)  # Replace with code to fit the model
    slope = float(model.coef_[0][0])  # Replace with code to extract slope from the fitted model
    intercept = float(model.intercept_[0])  # Replace with code to extract intercept from the fitted model
    ...
    for _ in range(S):
        ...
        X_sim =  np.random.rand(N, 1)  # Replace with code to generate X values

        # TODO: Generate Y values with normal additive error (mean mu, variance sigma^2)
        Y_sim = mu + np.sqrt(sigma2) * np.random.randn(N, 1)  # Replace with code to generate Y values
        ...
        slopes.append(sim_model.coef_[0][0])  # Replace None with code to append slope
        intercepts.append(sim_model.intercept_[0])  # Replace None with code to append intercept
